...
import com.procuratorate.app.bean.TaskWaitList;
 ...
 */
public class TaskWaitFrgAdapter extends  BaseListAdapter<OrderListBean> {
    private LayoutInflater inflater;
    public TaskWaitFrgAdapter(Activity context) {
        super(context);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if (convertView==null){
            convertView = inflater.inflate(R.layout.item_wait_task,null);
        ...
        if (item!=null){
            holder.tvCarDepartment.setText("申请部门："+item.usecardep);
            holder.tvCarUseTime.setText("开始时间："+item.usecarstartdate);
            holder.tvCarEndTime.setText("结束时间："+item.usecarenddate);
            if (item.status!=null){
